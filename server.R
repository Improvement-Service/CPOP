...
##Create Plot Outputs for page 2 & 3 ========================  
  ...
  output$CompCPP <- renderPlot({
    ...
    plts <- list()
    indi <- unique(dta$Indicator)
    plts <-lapply(1:18, FUN = function(.x){
      ggplot(data = filter(dta, Indicator == indi[[.x]])) +
        geom_bar(aes(x = reorder(CPP,-value), y = value, fill = colourscheme), 
  ...
  ##Render all Plots for Page 3
  output$SimCPP <- renderPlot({
    FGroup <- filter(CPPdta, CPP == input$LA1)[[1,6]]
    dta <- filter(CPPdta, Year %in% c("2016/17", "2014-2016") & FG %in% FGroup)
    dta$colourscheme <-ifelse(dta$CPP == input$LA1,"Sel1","Other")
    #lapply to generate plots
    plts <- list()
    cmp <- filter(CPPdta,Year %in% c("2016/17", "2014-2016") & CPP == "Scotland")$value
    indi <- unique(dta$Indicator)
    plts <-lapply(1:18, FUN = function(.x){
      ggplot(data = filter(dta, Indicator == indi[[.x]])) +
        geom_bar(aes(x = reorder(CPP,-value), y = value, fill = colourscheme), 
...
##Create Leaflet Maps=============================
  output$IZUI <- renderUI({
    selectizeInput("IZ", "", choices = sort(unique(CPPMapDta[CPPMapDta$council == input$LA1, 11])),
